...
- `src/hooks/useFaceLandmarker.ts`: MediaPipe model loading, tolerant per-frame face detection, short track retention, and stable ID assignment.
- `src/hooks/useAttentionSession.ts`: Session state, attention timeline, and aggregate metrics.
- `src/lib/attentionClassifier.ts`: Attention classification utilities.
...
4. Landmark data is converted into bounding boxes, matched to stable local track IDs, and briefly retained when detections drop so downward-looking faces do not disappear immediately.
5. Session hooks aggregate live attention metrics and feed the dashboard components.
6. Camera tracks and model loops are shut down explicitly when the user stops the camera or the app unmounts.
7. A service worker caches the app shell and versioned assets so the site can be installed and reopened as a standalone web app.
...
- Asset base is set to `/studenstrack/` so production files resolve correctly when deployed under that route.
