...
- Attention tracking in the live summary, timeline, and session export without mood or emotion detection.
...
The Vite build is configured with the `/studenstrack/` base path so the generated production assets resolve correctly when the application is deployed under that route.

## Installable App

The project ships a web app manifest, app icons, and a service worker so compatible browsers can install it as a standalone application under `/studenstrack/`.
